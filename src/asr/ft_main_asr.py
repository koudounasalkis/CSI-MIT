...
def parse_args ():
    ...
    parser.add_argument('--approach', type=str, default='csi')
    parser.add_argument("--balancing", action="store_true", required=False)
...
if args.balancing:
    #################### Add the held out dataset to the train dataset ####################
    with open(f'divergent_samples_librispeech_{args.approach}.txt','rb') as f:
        ids = f.read().decode('utf-8').split('\n')
        new_samples = train_dataset_held_out.filter(lambda example: example['id'] in ids)
        train_dataset = concatenate_datasets([train_dataset, new_samples])
...
output_dir = f"results/librispeech/{args.approach}"
